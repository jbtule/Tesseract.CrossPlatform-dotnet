...
namespace Tesseract
{
    public unsafe sealed class Pix : DisposableBase, IEquatable<Pix>
    {
        ...
        public static Pix LoadFromMemory(byte[] bytes)
        {
	        ...
	        {
		        handle = Interop.LeptonicaApi.pixReadMem(ptr, bytes.Length);
	        }
        ...
        {
            IntPtr handle;
            handle = Interop.LeptonicaApi.pixReadFromMultipageTiff(filename, ref offset);
        ...
        {
            IntPtr resultHandle = Interop.LeptonicaApi.pixConvertTo8(handle, cmapflag);
